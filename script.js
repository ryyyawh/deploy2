...
function showSkill(skillId) {
    ...
    if (target) {
        ...
        const defaultSkill = document.getElementById('default-skill');
        if (defaultSkill) {
            defaultSkill.classList.add('active');
        }
    }
}
...
function stopAllMusic() {
    ...
    if (musicBtn1) musicBtn1.textContent = '▶ Play Cincin';
    if (musicBtn2) musicBtn2.textContent = '▶ Play Mata Air';
    if (musicBtn3) musicBtn3.textContent = '▶ Play Evaluasi';
}
...
function updateNowPlaying(title, artist, image) {
    ...
    if (songImage) songImage.src = image;
